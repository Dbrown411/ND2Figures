...
class SamplePlotter:
    ...
    def sample(self, sample):
        ...
        self.fig_out = self.fig_folder / f"{self.figname}"
        self.offset_out = self.offset_folder / f"{self.figname}"
    ...
    def calculate_normmax(self):
        ...
        if (self.norm_across_samples) & (self.norm_across_wavelengths):
            maxes = [
                x for c, x in self.channel_maxes.items()
            ...
            for c, v in channel_norm.items():
                ...
                channel_norm[c] = int(
                    np.round((np.divide(cur_max, overall_max) * 255)))
            ...
            indices = [(c, self.sample.channel_order[c]) for c in normalize
                       if c in self.sample.channel_order.keys()]
            if len(indices) > 1:
                ...
                overall_max = np.max(maxes)
                norm_against = [
                    (c, int(np.round((np.divide(x, overall_max) * 255))))
                    for c, x in norm_against
                ]
    ...
    def plot_channel(self, c):
        ...
        color_frame = cv2.normalize(color_frame,
                                    None,
                                    0,
                                    channel_max,
                                    cv2.NORM_MINMAX,
    ...
    def _calc_figsize(self):
        ...
        if framesize[1] > framesize[0]:
            ncols = 1
            nrows = n
            self.figsize = (framesize[1] / self.ppi,
                            (framesize[0] * nrows / self.ppi) * (1 / .9))
        else:
            ncols = n
            nrows = 1
            self.figsize = (framesize[1] * ncols / self.ppi,
                            (framesize[0] / self.ppi) * (1 / .9))
